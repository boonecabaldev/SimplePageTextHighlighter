(function () {
  ...
  const HighlighterView = {
    ...
    highlightRange(range) {
      ...
      try {
        ...
        const textNodes = HighlighterModel.getTextNodesInRange(range);
    ...
    unhighlightRange(range) {
      ...
      spans.forEach((span) => {
        ...
        const intersection = HighlighterModel.getIntersection(range, spanRange);
  ...
  function highlightSelectedText() {
    ...
    if (selection && selection.rangeCount > 0) {
      ...
      if (!selectedText) {
        HighlighterView.showAlert(
          "Please select some text to highlight or unhighlight!"
        );
        return;
      }

      const isAlreadyHighlighted =
        HighlighterModel.hasHighlight(range.startContainer) ||
        HighlighterModel.hasHighlight(range.endContainer);

      if (isAlreadyHighlighted) {
        HighlighterView.unhighlightRange(range);
      } else {
        HighlighterView.highlightRange(range);
      }
